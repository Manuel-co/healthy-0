  ...
  return (
    <div className="bg-[#e8e4dc] px-3 pb-3 sm:px-4 sm:pb-4 lg:px-5 lg:pb-5">
      <div className="w-full rounded-[20px] lg:rounded-[28px] overflow-hidden bg-[#f0ede6] px-6 py-12 md:px-10 md:py-16">
        ...
        {/* Header */}
        <div className="mb-10">
          <p className="text-[11px] tracking-[0.15em] uppercase text-[#1a1a1a]/40 mb-3">What clients say</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {testimonials.map((t, i) => (
            <div key={i} className="bg-white rounded-2xl p-6 flex flex-col justify-between">
              {/* Stars */}
              <div className="flex items-center gap-1 mb-4">
                {Array.from({ length: t.rating }).map((_, j) => (
                  <svg key={j} width="13" height="13" viewBox="0 0 24 24" fill="#e8c96a">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
          {[
            ...
            <div key={i} className="bg-white rounded-2xl px-6 py-5">
              <p className="text-[28px] sm:text-[32px] font-light text-[#1a1a1a] leading-none mb-1">{stat.value}</p>
