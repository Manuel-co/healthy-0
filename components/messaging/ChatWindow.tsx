...
import { getSessionById, isSessionActive, msRemaining } from "@/lib/sessions-data";
...
export function ChatWindow({
  ...
  async function handleSend(text: string, imageUrl: string | null) {
    setSendError(null);
    try {
      const optimistic = await sendMessage(sessionId, currentUserId, text, imageUrl);
      setMessages((prev) => [...prev, optimistic]);
    } catch (err) {
      setSendError(err instanceof Error ? err.message : "Couldn't send that message.");
      // The send was rejected because the session just locked — reflect that now.
          ...
          {session && (
            <p className={cn("flex items-center gap-1 text-[11px]", warning ? "font-medium text-destructive" : "text-muted-foreground")}>
              {active ? (
                ...
                `Session ended · ${formatRelativeTimestamp(session.endedAt ?? session.expiresAt)}`
              )}
            </p>
          )}
        </div>
